...
const resolve = mod => require.resolve(mod); // eslint-disable-next-line complexity


var _default = function _default() {
  let config = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  const staticPath = (0, _getStaticBasePath.default)(config);
  ...
  const reactPath = _resolveFrom.default.silent(basePath, 'react');

  const reactDomPath = _resolveFrom.default.silent(basePath, 'react-dom');
  ...
  const babelConfig = tryRead(_path.default.join(basePath, '.babelrc'));
  const isProd = env === 'production';
  const cssExtractor = new _extractTextWebpackPlugin.default({
  ...
  const commonChunkPlugin = (typeof config.commonChunkPlugin === 'undefined' || config.commonChunkPlugin) && new _webpack.default.optimize.CommonsChunkPlugin({
    name: 'vendor',
